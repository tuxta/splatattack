...
class HighScores(Level):
    def __init__(self, screen, joysticks):
        ...
        print("Global Name: {}  Global Score: {}".format(Globals.player_name, Globals.SCORE))

        database.add_score(Globals.player_name, Globals.SCORE)

        names_scores = database.get_top_5()

        self.add_room_object(TextObject(self, 100, 50, 'Top 5 Scores', 80))
        y_pos = 150
        for person in names_scores:
            self.add_room_object(TextObject(self, 100, y_pos, person[0]))
            self.add_room_object(TextObject(self, 300, y_pos, str(person[1])))
            y_pos += 70
