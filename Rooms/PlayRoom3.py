...
class PlayRoom3(Level):
    ...
    def add_ant(self):
        if Globals.num_ants < 3:
            Globals.num_ants += 1
            self.add_room_object(Insect(self, 0, 0, self.splat_sound, self.ant_image, self.splat_image, 5, 25))
            self.set_timer(120, self.add_ant)

    def add_roach(self):
        if Globals.num_roaches < 4:
            Globals.num_roaches += 1
            self.add_room_object(Insect(self, 0, 0, self.splat_sound, self.roach_image, self.splat_image, 6, 50))
            self.set_timer(240, self.add_roach)

    def add_scorpion(self):
        if Globals.num_scorpions < 4:
            Globals.num_roaches += 1
            self.add_room_object(Insect(self, 0, 0, self.splat_sound, self.scorpion_image, self.splat_image, 8, 75))
            self.set_timer(120, self.add_scorpion)

    def add_spider(self):
        if Globals.num_spiders < 2:
            Globals.num_spiders += 1
            self.add_room_object(Insect(self, 0, 0, self.splat_sound, self.spider_image, self.splat_image, 10, 100))
